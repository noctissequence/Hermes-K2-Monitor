...
def sign(mesh_key: bytes, node_id: str, op: str, payload: dict, nonce: str, ts: float) -> dict:
    material = f"{node_id}|{op}|{canonical(payload)}|{nonce}|{ts}".encode()
...
def main() -> int:
    ...
    if not (status == 200 and "agents" in state and "tasks" in state and "collab" in state):
    ...
    if not (status == 200 and invite.get("code")):
        raise RuntimeError((status, invite))
    ...
    if not (status == 200 and credentials.get("token")):
        raise RuntimeError((status, credentials))
    ...
    if not (status == 200 and relay_result.get("status") == "ok"):
        raise RuntimeError((status, relay_result))
    ...
    material = f"{args.node_id}|broadcast|{canonical(ledger_payload)}|{nonce}|{ts}".encode()
    ...
    if not (status == 200 and isinstance(ledger.get("ledger"), list)):
        raise RuntimeError((status, ledger))
